...
export default function Home() {
  ...
  const handleDownloadImage = async () => {
    ...
    // Facem o copie a calendarului, ascunsă în afara ecranului, forțată pe
    // 3 coloane fixe — la fel ca pe calculator — indiferent de pe ce
    // dispozitiv (telefon sau PC) se apasă butonul. Astfel poza generată
    // arată mereu la fel, nu ca varianta "1 coloană" de pe mobil.
    let wrapper: HTMLDivElement | null = null;
    try {
      const clone = calendarRef.current.cloneNode(true) as HTMLDivElement;
      clone.removeAttribute('class');
      clone.style.display = 'grid';
      clone.style.gridTemplateColumns = 'repeat(3, minmax(0, 1fr))';
      ...
      wrapper.style.background = '#0a0f1e';
      wrapper.style.padding = '24px';
      wrapper.appendChild(clone);
      document.body.appendChild(wrapper);

      // un frame de layout/paint înainte de captură
      ...
      canvas.toBlob((blob) => {
        ...
        deliverFile(blob, fileName, 'photo');
      }, 'image/png');
    } catch (err) {
        ...
        {!schedule ? (
          ...
          <div className="space-y-8">
            ...
            <div ref={calendarRef} className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {schedule.slice(0, exportMonths).map((monthData, mIdx) => {
